...
상대 라이너가 **새 아이템을 완성**하면 화면 위 오버레이에 **아이템 아이콘 + 한 줄 텍스트**로 알린다.

- 데이터: Overwolf `overwolf.games.events`의 `live_client_data` 피처 → `all_players`에 각 플레이어 아이템(`itemID`, `displayName`) 포함. **Riot 프로덕션 키 불필요**(로컬).
- TTS·팀 전적 분석은 2차 (Riot 프로덕션 키 승인 후).
